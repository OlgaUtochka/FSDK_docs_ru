...
* Набор *fda* на данный момент является самым новым. В отличие от предыдущих наборов, он обеспечивает высокое качество в широком диапазоне ракурсов (вплоть до профильных). Однако алгоритмы распознавания по-прежнему требуют, чтобы ракурс лица было как можно ближе к фронтальному. Набор *fda* содержит 21 точку.
